...
| [T-201](T-201-bff-cv-aggregate.md) | BFF: aggregated public CV endpoint — H1 ratified, implemented, A1 + repo CI green, **review round 1 applied** (a `Promise.all` race let a section's 404 beat the person's and answer 502 where the contract says 404) and **exploratory QA PASSED** (75/75 404s under parallel load; no id/email leak against real data) — **at H2** | cv-bff-node | in_review | fullstack-developer | T-101…T-104, T-006 | [#5](https://github.com/erfeamor/cv-bff-node/pull/5) |
