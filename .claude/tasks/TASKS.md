...
| [T-207](T-207-public-types-derived-from-domain-interfaces.md) | BFF: the aggregate's `Public*` types are `Omit<Domain*, …>` — so `tsc` checks the public payload against the **upstream shape it exists to distrust**, and the guarantee fails **both** ways (a required domain field pressures the leak back in; a new optional contract field is silently dropped) | cv-bff-node | todo | | T-205 ✔ | |
