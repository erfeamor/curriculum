...
**Filed as work, not drift:** [T-024](T-024-contract-skill-assignment-put-shape.md) was implemented in this session rather than left on the board, because it is a docs-only contract change and landing it before T-103 means the implementer builds from `docs/api-contract.md` instead of from a ruling buried in a task file — which is the arrangement the contract exists to replace.
