...
| [T-201](T-201-bff-cv-aggregate.md) | BFF: aggregated public CV endpoint — **MERGED**. Full pipeline: 5 stage-0 rulings, H1, A1, CI, review round 1 (caught a `Promise.all` race answering 502 where the contract says 404 — the existing test could not see it), exploratory QA (75/75 404s under parallel load, no id/email leak against real data), H2 | cv-bff-node | done | fullstack-developer | T-101…T-104, T-006 | [#5](https://github.com/erfeamor/cv-bff-node/pull/5) |
| [T-205](T-205-bff-allowlist-section-normalizers.md) | BFF: the aggregate's four section normalizers are **denylists** (`...rest` passes undeclared upstream fields through) where every other normalizer in the repo is an allowlist — on the one route with no auth in front of it | cv-bff-node | done | fullstack-developer | T-201 ✔ | [#6](https://github.com/erfeamor/cv-bff-node/pull/6) |
| [T-206](T-206-person-id-guard-numeric-overflow.md) | The shared person-id guard accepts digit runs that overflow Java `Long` — 5 upstream calls and a **502 where a 400 belongs**, and its doc comment promises a guarantee the code does not keep | cv-bff-node | done | fullstack-developer | T-201 ✔ | [#7](https://github.com/erfeamor/cv-bff-node/pull/7) |
| [T-207](T-207-public-types-derived-from-domain-interfaces.md) | BFF: the aggregate's `Public*` types are `Omit<Domain*, …>` — so `tsc` checks the public payload against the **upstream shape it exists to distrust**, and the guarantee fails **both** ways (a required domain field pressures the leak back in; a new optional contract field is silently dropped) | cv-bff-node | in_progress | fullstack-developer | T-205 ✔ | |
| [T-208](T-208-error-handler-status-and-metrics-cardinality.md) | The terminal error handler flattens **every** non-auth error to 500 — including Express's own decode `400` — so an anonymous caller mints 5xx + stack logs at will; and unmatched requests echo `req.path` into a Prometheus label, making cardinality **attacker-driven** | cv-bff-node | todo | | — | |
...
| [T-026](T-026-first-build-after-cold-start-fails.md) | First build after a Jenkins restart fails — **FIXED**. Root cause was a JENKINS-23152 build-number collision: the DSL seed recreated the jobs on every boot against a persistent `JENKINS_HOME`. Attempt 1 (item-model guard) was proven INERT — JCasC runs the DSL before jobs load; attempt 2 probes the job's `config.xml` on disk instead. **Verified on the live host** (`createOrUpdateConfig` 0, JENKINS-23152 0, clean cold-start push to a branch with an existing `builds/1` went green) | cv-infra | done | infrastructure-engineer | T-019 | [#21](https://github.com/erfeamor/cv-infra/pull/21) |
...
| — | [T-204](T-204-bff-validate-person-id-param.md) | BFF: validate the person id before the upstream call — **adopts T-201's shared guard, does not write a second one**; **T-206 ✔ merged first (2026-08-27), so the guard it adopts is already length- and value-bounded** | cv-bff-node | done | fullstack-developer | T-202 ✔, **T-201 ✔** | [#8](https://github.com/erfeamor/cv-bff-node/pull/8) |
