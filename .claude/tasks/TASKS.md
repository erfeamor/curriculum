...
| [T-206](T-206-person-id-guard-numeric-overflow.md) | The shared person-id guard accepts digit runs that overflow Java `Long` — 5 upstream calls and a **502 where a 400 belongs**, and its doc comment promises a guarantee the code does not keep | cv-bff-node | todo | | T-201 | |
...
| — | [T-204](T-204-bff-validate-person-id-param.md) | BFF: validate the person id before the upstream call — **adopts T-201's shared guard, does not write a second one** | cv-bff-node | todo | | T-202, **T-201** | |
