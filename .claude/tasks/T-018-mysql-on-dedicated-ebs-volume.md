...
checkpoint:
  ...
  note: "SUPERSEDED 2026-08-24 — this described the task's pre-claim state and contradicts `stage: done`, `merged` and `outcome` ABOVE it in the same mapping; the `claimed` key immediately below already recorded that it had been overtaken, but this key was never marked. Kept verbatim per strike-don't-delete. It read: 'NOT a fresh todo. Stage 0 refinement was completed 2026-08-13 and the DoR rulings below are written up; whoever picks this up starts at IMPLEMENTATION, not refinement. Deliberately left status:todo with no owner so the board cannot repeat the stale-claim bug that parked three wave-1 tasks earlier this session — an H1-complete task with an owner set reads as in-flight and blocks re-pickup under board rule 1.' The reasoning about stale claims remains sound and is worth carrying to future tasks; it is only this task's status that moved."
