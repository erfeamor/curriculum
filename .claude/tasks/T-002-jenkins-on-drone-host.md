...
checkpoint:
  ...
  post_apply:
    ...
    still_open: "ONE item: the Drone OAuth CALLBACK round trip. The redirect out is verified (/login → 303 → github.com/login/oauth/authorize with correct client_id/state/scopes); the return leg through the proxy needs a real GitHub session and automation cannot cover it. Also unmeasured though not a gate: memory during a real Maven build concurrent with a Drone pipeline."
