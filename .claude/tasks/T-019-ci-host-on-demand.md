...
- [x] The host stops automatically when idle, and **a build in progress is never killed** — proven by starting a build and confirming it survives the shutdown window.
  - **SETTLED 2026-09-24 ([T-111](T-111-domain-service-jenkins-pipeline-timeout.md)'s QA record).** A deliberately quiet build (`sleep` inside the Test stage, 35-minute bound) outlasted the 20-minute CPU window. Three consecutive reaper checks (21:49, 21:54, 21:59Z) logged `cpu quiet: peak 4.7–5.0%` and then **`busy: 1 executor(s) running`**, and declined to stop. So the executor read alone kept the build alive, which is the half this criterion was waiting on. The host stopped at the next check after the build ended.
  - **Observed at the same time — the reaper is not what Ruling 2 describes.** It stopped the host **~1.5 min** after the last build ended (build over 22:02:43, stop 22:04:15). Its *"after 20 idle minutes"* is the CPU window plus one idle executor read with a re-check, **not** 4 consecutive idle executor checks. That is harmless in practice, because a push arriving after the stop re-wakes the host through the doorbell. But anyone reasoning from Ruling 2's *N = 4* should know the actual behaviour.
  ...
  - **Observed 2026-09-24, not settled ([T-153](T-153-jenkins-deploy-stage-dead-gate-and-rds.md)'s QA record).** A 10-minute forced-hang build survived three reaper checks. But those checks declined on `2 item(s) queued` and then on the **CPU veto** (`CPU peaked at 36.6% over 20 min`), never on `busyExecutors`. The reaper then stopped the host cleanly at 12:09Z, 20 idle minutes after the build. A clean test needs a busy-but-quiet build that outlasts the 20-minute CPU window.
