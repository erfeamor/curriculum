...
- **Related but separate:** the cost model in `cv-infra/CLAUDE.md` (~$0.92/day ≈ $28/month) and T-010's runway are both stale since the 2026-08-08 `t3.micro`→`t3.small` resize took the real rate to ~$1.23/day ≈ $37.30/month. Correcting those is not this task, but this task's saving should be folded in when they are.
