...
checkpoint:
  ...
  APPLIED: true             # applied 2026-08-13 ~17:0x local; backup verified end to end, see applied_note
  applied_note: "Applied 2026-08-13. Plan matched the PR's expected shape: 7 added, 0 changed, 2 destroyed. Instance replaced (i-06dca5eba9351b75a -> i-038600c71d141035b); EIP 15.236.195.130 preserved, only the association was recreated. Two deviations from the runbook's prediction, both benign and understood: aws_eip_association was REPLACED rather than updated in place (replace_because_cannot_update — the runbook's wording was wrong, the EIP itself never moved), and local_file.jenkins_provision_script was created, which is unrelated T-002 local drift (./generated/ missing from this clone). null_resource.jenkins_provision stayed no-op, so the CI host was untouched."
  verification: "Done end to end, not assumed. (1) umask 077 confirmed present at line 10 of /usr/local/bin/mysql-backup.sh on the live box — the security fix is deployed. (2) mysql-backup.timer enabled, next run 2026-08-14 03:04 UTC. (3) Forced a run: service Finished cleanly, temp file removed, no /tmp leftover. (4) s3://cv-project-mysql-backup-dev/mysql-dumps/cv-20260813T152654Z.sql.gz landed, 1557 bytes. (5) RESTORE VERIFIED — downloaded, gunzipped, restored into a throwaway mysql:8.4 container: all six schema tables plus flyway_schema_history present, 1 migration row. (6) Bucket posture confirmed against the live account: all four public-access-block flags true, lifecycle expire-old-mysql-dumps at 7 days scoped to mysql-dumps/."
  dump_is_schema_only: "The verified dump contains schema and the Flyway history, no rows — the instance replacement wiped the test data and production applies migrations only (dev-seeds are dev-only by design). The backup mechanism is proven; it has simply not yet had authored content to capture. This is the expected state, not a defect."
  deferred_finding_observed: "The mysqldump password warning appears in the service journal exactly as the security review predicted: 'Using a password on the command line interface can be insecure.' Confirms the deferred -p finding is real and visible. Still non-blocking on a single-tenant SSM-only box; fix with --defaults-extra-file before that changes."
  repo: cv-infra
  branch: feat/mysql-backup-to-s3
