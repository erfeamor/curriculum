...
- **[T-155](T-155-flyway-version-supports-mysql-84.md)** — **DECIDED 2026-09-24: bump to `13.7.0`; this task carries the production pin (AC below), and `T-156` is now in `depends_on` so production moves only after the CI gate has proven 13.** Original note: if H1 there decides to bump Flyway, its production pin lives in `templates/domain-service-user-data.sh:181`, the same file this task edits, and `compute.tf:63` (`user_data_replace_on_change = true`) makes any change there an instance replacement. Carrying it in **this** apply costs no extra replacement. **Do not merge that pin to `cv-infra` master ahead of this task:** cv-infra is one root module with local state, so a merged-but-unapplied change rides the *next* `terraform apply` of any cv-infra task (T-021, T-005, T-007, T-033 …), whose plan would suddenly show a replacement it never asked for. Same PR, or merged back-to-back with no other cv-infra apply between. Order of the bump itself: dev + CI pins first, production last — and no new migration until all four agree (T-155 § *Empirical answers*).

Neither was a `depends_on` edge when written. ~~this task is the claimable head of the chain and must not wait on either~~ **Since 2026-09-24 T-156 is one** (the bump is decided and production must move last); T-208 merged the same day.

## RAM is now a cost decision, not only a stability one (added 2026-09-25, board review)

The RAM watch-out below says to argue a resize *"on stability, not cost"*. That was right on the Free plan. **Under T-012's decision A it no longer holds.** The trimmed ~$13/month model assumes the app host stays at **1 GB**, and [T-035](T-035-app-host-to-graviton.md) plans a 1 GB `t4g.micro` after this task. If this task finds it needs a `t3.small` (2 GB), that breaks both: it adds ≈ $8.6/month, about two-thirds of the trimmed bill, and it removes T-035's premise.

**So stage 4 must measure memory under a warm JVM plus the BFF** (`free -m`, swap in use, `docker stats`). T-035's AC already compares against *"T-014's numbers on `t3.micro`"*, and this is where they come from. **Any resize is decided at H2 with that measurement and its cost in front of the human**, never folded silently into the apply.
