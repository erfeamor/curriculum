...
.claude/tasks/
  ...
  HISTORY.md    ← board consistency sweeps and close-out narratives (read when investigating drift)
...
See [TASKS.md](TASKS.md) for all three — **start at its *Now / Next / Later* lane**, which orders the claimable work; `depends_on` stays authoritative wherever the two disagree. Before trusting any prose you find in a task file that has been open a while, read [HISTORY.md](HISTORY.md) — it contains the **board consistency sweeps** (2026-08-17, two on 2026-08-20, 2026-08-22, and 2026-09-23) and narratives from major task merges. Each sweep records prose that was true when written and was never re-checked; the convention is to strike rather than delete, so the corrections are findable and the original error stays readable.
