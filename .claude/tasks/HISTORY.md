...
## Moved out of TASKS.md in the 2026-09-23 board review — verbatim

TASKS.md had regrown to 41 KB against the README's *"one line per task"*. These blocks are moved unchanged; each was either a merge narrative or reasoning about a state that no longer holds. Row titles were also cut back to titles — the pre-trim rows are at `git show 85962e4:.claude/tasks/TASKS.md`. Current-state summaries of what matters here replaced them on the board.

### Parallelization notes (TASKS.md § M2, as of 2026-09-22)

- **Wave 1 complete:** T-101 (`09282ed`), T-102 (`42abe91`), T-103 (`2e54394`), T-104 (`7677fee`), T-151 (`865784f`) all merged. **Wave 2 unblocked:** T-201 and T-301 may start against the contract (mocked upstreams). **Wave 3:** T-401 and T-402 after T-201 (run in parallel); T-501 strictly last. ~~**T-105** is still `todo` and blocks T-501 — it is the only task making Experience contract-compliant on ordering.~~ **T-105 MERGED 2026-08-26** (`1b9b398`, [#9](https://github.com/erfeamor/cv-domain-service/pull/9)) — all four section collections are now contract-compliant on ordering, and **T-501's last M2-domain blocker is gone**. What still gates T-501 is the deployment chain (T-014 → T-403/T-404) plus T-201/T-301/T-401/T-402. ~~**[T-201](T-201-bff-cv-aggregate.md) is the head of everything remaining**: it is claimable today and T-014 waits on it.~~ **T-201 MERGED 2026-08-27** (`65f6c0a`, [#5](https://github.com/erfeamor/cv-bff-node/pull/5)) — **the head of the chain is gone and four tasks unblocked with it: T-014, T-401, T-402 and T-204.** [T-014](T-014-deploy-bff-to-aws.md) is now the head of everything remaining, and it is the expensive one (real apply + stage-4 AWS verification; adapter §7 says budget the full ceiling and never run it in a wave).
- **T-205 MERGED 2026-08-27** (`68dbfd2`, [#6](https://github.com/erfeamor/cv-bff-node/pull/6)) — the aggregate's four section normalizers are allowlists, so cv-domain-service's entity shape is no longer the BFF's public anonymous payload. **Filed from its review round: [T-207](T-207-public-types-derived-from-domain-interfaces.md)**, now unblocked. The finding is worth reading before anyone touches `cv.ts` again: T-205 fixed the *runtime*, but the **types are still a denylist** — `PublicExperience = Omit<DomainExperience, 'id'>` means `tsc` checks the new rebuilds against the very shape the task exists to distrust, and it fails **both** ways (a required domain field pressures the leak back in with a green build; a new *optional* contract field is silently dropped). Both directions were verified empirically, twice. Its stage-4 QA also produced the first **live record of what cv-domain-service actually sends** per section — nothing in cv-bff-node pins those keys, which is the whole point — and confirmed no `personId`/`email` reaches the wire today, so T-205's "not currently exploitable" framing is now measured rather than asserted.

- **T-207 MERGED 2026-08-28** (`dc21c27`, [#9](https://github.com/erfeamor/cv-bff-node/pull/9)) — the four `Public*` types are now standalone interfaces transcribed from `docs/api-contract.md`, so `tsc` checks the anonymous payload against the **contract** instead of against the upstream it exists to distrust. Both directions are closed and each was re-probed by the driver at three separate stages: a required field added to a `Domain*` interface is now a **no-op** (it was `TS2741` pressure to copy the leak back in), and a new **contract** field is `TS2741` until the matching `strip*` copies it. That second half only works because every contract-optional field is declared a **required key with a nullable value** (`location: string | null | undefined`), never `?:` — H1 caught that the obvious `?:` transcription would have satisfied the acceptance criterion while closing neither direction, which is this board's signature failure mode reproduced *inside the task written to fix it*.
  **Two further instances of that same shape surfaced during the work and are worth reading:** (1) the developer's first guard could not catch a field regressing from `| undefined` to `?:` — one `@ts-expect-error` covering two omissions stays "used" while either remains required — and it passed every other probe, so nothing else in the pipeline would have caught it; (2) review found `normalizePerson` had **no declared return type**, so its result was spread into `body` with no excess-property check and `email: person.email` typechecked **clean** — the one field this repo ranks a hard blocker — while the four section normalizers *were* protected. Both fixed; both pinned by tests.
  **Filed from its review round: [T-209](T-209-contract-optional-field-null-semantics.md)** — the contract never says whether an optional field arrives as `null` or absent, three repos hold three different answers, and cv-domain-service's tests **cite the contract for a rule it does not contain**. That is why the types here read `string | null | undefined` rather than the intended `string | null`; once T-209 corrects the `Domain*` interfaces they can tighten.
  Stage 4 ran **no live stack, deliberately and with evidence**: both trees compiled with `tsc --removeComments` emit **byte-identical** JavaScript, so there is no runtime delta to exercise and a green smoke would have measured nothing.


- **T-209 / T-210 / T-405 MERGED 2026-08-28** — the null-vs-absent line, closed contract-first across three repos in one run. **[T-209](T-209-contract-optional-field-null-semantics.md)** (`2230f59`, [#83](https://github.com/erfeamor/curriculum/pull/83)) added **design rule 7**: an optional field is always a PRESENT key whose empty value is `null`. **[T-210](T-210-bff-domain-types-null-not-absent.md)** (`005337b`, [#10](https://github.com/erfeamor/cv-bff-node/pull/10)) and **[T-405](T-405-public-react-null-optionals.md)** (`ffa5244`, [#4](https://github.com/erfeamor/cv-public-react/pull/4)) then typed the two consumers to the rule. Both are zero-runtime-delta changes; T-210's entire compiled output is byte-identical to its parent.
  **Read rule 7's carve-outs before writing a consumer**, because both were wrong in a draft and caught at review: **requests are the opposite** (`PUT` replaces, so an omitted optional in a *request* body IS the empty case), and **`endDate` is not governed by rule 7 at all** — absent from every `Required:` list but always emitted, its `null` meaning "current" under rule 3. The end-to-end paragraph is also deliberately phrased as **inheritance, not enforcement**: the BFF copies verbatim and has no `?? null`, so the guarantee rests on the producer. An earlier draft asserted it as a BFF guarantee — which would have been a sentence that looks like a guarantee and enforces nothing, written into the task closing a specification gap.
  **T-210 grew a second route at review.** `cv-bff-node/src/routes/people.ts` held an independent `DomainPerson` and a `PublicPerson` with `?:` keys — so T-207's drop direction was **still open on `GET /bff/api/v1/people/:id`**, anonymous by contract. Verified red-before-green: omitting `headline` from `normalize()` typechecked CLEAN on `dc21c27`. Widened rather than filed, because the AC was already repo-wide.
  **Two guards in this run passed for the wrong reason and were caught by probing them, not by reading them** — the lineage's recurring shape: (1) `NoOptionalKeys` does **not** catch a required key typed `string | undefined`, so the spelling T-210 removed was reintroducible with a green build until `NoUndefinedValues` was added; (2) T-210's first head-route coercion test used a *mixed* fixture and `null || null` is still `null`, so it caught a coercion only on whichever field happened to be `''`. **Filed from T-405's review: [T-406](T-406-public-react-bff-path-missing-prefix.md)** (cv-public-react fetches a path the BFF does not serve — and its test asserts that wrong URL, so the suite is green) and **[T-407](T-407-public-react-tocv-null-invariant.md)** (`toCv` never establishes the invariant T-405's types assert — a type made stronger without being made true). **[T-402](T-402-public-react-cv-sections.md) now depends on T-405**, so the section types cannot be rendered against before they were corrected.
...
### Public-path deployment gap — resolved-blocker prose and chain corrections
...
One task per repo. The **numbered** rows are strictly sequential and their `depends_on` enforces the order; the unnumbered rows hang off the chain and are claimable independently once their own dependency is met. **This is the board line for all eight; claim here.** (Corrected 2026-08-17: this said "exactly one is claimable at a time", which stopped being true when T-202 merged — ~~T-204 became claimable alongside T-014~~, and T-404 now joins it. **T-204 correction, 2026-08-27:** it gained `T-201` in `depends_on` during T-201's review round, so it was briefly **not** claimable — T-201 ships the shared guard this task adopts. **T-201 merged the same day, so T-204 is claimable again**, and the guard now exists on `master` for it to use. Read [T-206](T-206-person-id-guard-numeric-overflow.md) first: it bounds that same guard, and landing T-204 before it means inheriting the weaker version.)
...
~~**T-014 is next in the chain and IS claimable.** Its `depends_on` is `[T-013, T-202]` and both are `done`.~~ **Stale again — corrected 2026-08-24, and note the irony: the 2026-08-24 session that added `T-201` to T-014's `depends_on` updated the task file and the chain table but not this paragraph, the exact failure this same paragraph's closing sentence warns about.** Current state: T-014's `depends_on` is `[T-013, T-202, T-201]`; the first two are `done`, **T-201 is `todo`, so T-014 is NOT claimable until T-201 merges** — a deliberate sequencing decision (the first deployed image must carry the aggregate; see T-014's frontmatter note). ~~**T-201 is what is claimable now**, and it is the head of the whole chain.~~ ~~**STALE — corrected 2026-08-27. T-201 is `in_review`**, so it is not claimable.~~ **SUPERSEDED THE SAME DAY: T-201 is `done`, merged as `65f6c0a`.** So the sentence this paragraph has now been wrong about four separate times finally resolves: **T-014's `depends_on` is fully satisfied and T-014 IS claimable.** **This paragraph has now gone stale about T-201 FOUR times** — twice about its dependencies, then about its status, then about that correction — and only the last was caught by the writer. Every other time a review round or a later session found it. The closing sentence below has been right every time and unheeded every time. This paragraph previously said the opposite — that T-014 "is NOT claimable yet" because it gated on T-001 §1, "which is still `todo` and unowned" — and both halves of that were false by the time it was written: T-001 is `done` (applied and restore-verified 2026-08-13), and T-001 had already been **removed** from T-014's `depends_on` that same day for the reason recorded at the end of this section. **Corrected 2026-08-14.** This is the T-002 board-line bug repeating one day later: prose that gates a task, contradicted by the frontmatter it claims to describe. When a dependency changes, the task file and *every* prose reference to it move together.

**[T-204](T-204-bff-validate-person-id-param.md) was filed from T-202's security review.** Pre-existing defect, but T-202 moved the route from JWT-gated to anonymous — so it goes public the moment T-014 deploys. It does not block T-014; it should not sit unfixed for long after it. **It now waits on T-201** (added 2026-08-27): T-201 predicted this task's defect would recur in the aggregate route, guarded against it there, and put the guard in a shared module *specifically* so T-204 adopts rather than re-implements it.

### Public-path deployment gap — personas-table bullets
