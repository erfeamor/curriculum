...
Host port for a service = <base published port> + slot * offset-step.

Usage:
    scripts/qa-env-override.py --task T-201 --slot 2
    scripts/qa-env-override.py --task T-201 --slot 2 --json
    scripts/qa-env-override.py --task T-201 --slot 0        # single-task run
...
def main(argv=None):
    ...
    ap.add_argument("--slot", type=int, default=0,
                    help="wave slot (0,1,2,…); host offset = slot * offset-step")
    ap.add_argument("--offset-step", type=int, default=10,
                    help="host-port offset per slot (default 10)")
    ...
    offset = args.slot * args.offset_step
    ...
    bff = next((m["published"] for svc, ms in report.items()
                if svc == "bff" for m in ms), None)
    smoke = (f"curl -fsS localhost:{bff}/api/v1/people/1/cv"
             if bff is not None else None)
    ...
    if args.json:
        ...
        print(f"project: {proj}  (slot {args.slot}, offset +{offset})")
